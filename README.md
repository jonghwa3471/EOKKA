...
Supabase Cron이 평일 한국시간 오후 2시 30분에 배포된 서비스의
`/api/cron/analyze-portfolios`를 `GET`으로 호출합니다. Supabase Cron에는
GMT 기준 `30 5 * * 1-5`를 사용하고, 요청 헤더에 아래 값을 추가합니다.
...
자동 분석은 새 종가가 확인된 경우 익명화된 포트폴리오 계산 요약으로 OpenAI
전략 분석도 함께 갱신합니다. 최근 30일 동안 홈이나 대시보드를 방문한 사용자만
자동 분석하며, 장기간 미접속 사용자는 시세 조회와 AI 호출을 건너뜁니다.
자동 분석의 목표금액과 매월 투자금은 `/dashboard/automatic-analysis`에서
별도로 저장합니다. 설정하지 않은 사용자는 기존처럼 대시보드 기준 목표를
사용하며, 선택값이 없으면 1억을 우선하고 1억 기록도 없으면 가장 작은 목표를
사용합니다. 설정한 목표의 기존 기록이 없어도 최신 활성 포트폴리오를 바탕으로
해당 목표의 첫 자동 기록을 생성합니다.
최신 시장 날짜가 기존 기록과 같으면 중복 저장하지 않으며, 사용자별·목표금액별
최신 30개 기록만 보관합니다. 기록 날짜는 분석을 실행한 날짜가 아니라 실제로
사용한 종가 기준일이며, 같은 종가 기준일에 다시 분석하면 기존 기록의 내용과
마지막 업데이트 시각만 갱신합니다.
