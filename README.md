...
일반 주식과 ETF·ETN을 모두 사용하려면 공공데이터포털에서 `금융위원회_주식시세정보`와 `금융위원회_증권상품시세정보` 두 API를 각각 활용신청해야 합니다. 버핏 원칙 AI가 국내 기업의 매출·이익·부채를 함께 살펴보게 하려면 `금융위원회_기업 재무정보`도 별도로 활용신청해야 합니다. 같은 공공데이터 일반 인증키를 사용할 수 있습니다.

```dotenv
FSC_STOCK_API_KEY=발급받은_일반_인증키
# 재무정보에 별도 키를 사용할 때만 설정합니다.
FSC_FINANCIAL_API_KEY=별도로_발급받은_일반_인증키
```
...
API 키가 없거나 OpenAI 호출이 실패하면 기존 수익률·시나리오 분석은 정상적으로 표시되고 AI 전략 요약만 생략됩니다. 국내 모드에서는 금융위원회 기업 재무정보에서 확인한 매출·이익·부채 지표를, 로컬 KIS 모드에서는 한국투자증권 현재가 상세의 PER·PBR·EPS·BPS를 종목명과 분리해 익명화한 뒤 AI에 전달합니다. AI는 제공된 수치만 해석하며, 개별 종목의 매수·매도를 지시하지 않습니다.
